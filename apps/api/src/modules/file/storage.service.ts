...
export class StorageService {
  ...
  async getTempFileUrl(fileId: string, maxAge?: number) {
    if (!this.isCloudPath(fileId)) {
      throw new Error('仅支持 cloud:// 云存储文件');
    }

    return this.wechatCos.getTempFileUrl(fileId, maxAge);
  }
